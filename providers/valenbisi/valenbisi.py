...
def get_stations():
    """
    Obtains the stations xml provided by http://www.valenbisi.es
        :rtype : list
    """
    try:
        response = urllib2.urlopen('http://www.valenbisi.es/service/carto')
...
def make_stations_jsonfile():
    ...
    if stations != 0:
        valenbisi_path = os.path.realpath(os.path.abspath(os.path.split(inspect.getfile(inspect.currentframe()))[0]))
        if not os.path.exists('%s/file' % valenbisi_path):
            os.mkdir('%s/file' % valenbisi_path)
        with open('%s/file/stations.json' % valenbisi_path, 'w') as jsonfile:
...
def load_stations():
    ...
    try:
        valenbisi_path = os.path.realpath(os.path.abspath(os.path.split(inspect.getfile(inspect.currentframe()))[0]))
        with open('%s/file/stations.json' % valenbisi_path, 'r') as stations_file:
...
def select_id(orion_id):
    """
    Select the valenbisi id
        :param orion_id: orion_id
        :rtype : valenbisi_id
    """
    pattern = "urn::Valencia:valenbisi"
    a = len(re.match(pattern, orion_id).group())
    return orion_id[a:]


def get_station_data(valenbisi_id):
    """
    Makes the valenbisi request
        :param valenbisi_id: valenbisi id number
        :rtype : list
    """
    url = 'http://www.valenbisi.es/service/stationdetails/seville/'+valenbisi_id
...
def get_data(_id, _type, max_time=1):
    ...
    try:
        ...
        entity.entity_add(_id, 'valenbisi', ispattern='false')
